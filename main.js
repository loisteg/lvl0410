...
const Targets = { [States.FOUR]: 4, [States.TEN]: 10, [States.THIRTY_EIGHT]: 38 };

// Тексти (українською)
const Copy = {
  start: "Натисни 4 рази",
  after4: "Щось не вийшло, давай ще раз, натисни 10 разів",
  after10: "Ну давай ще 38",
};
...
function spawnParticles(n) {
  for (let i = 0; i < n; i++) {
    ...
    span.style.animation = `particle-fly ${380 + Math.random() * 200}ms ease-out forwards`;
...
function finalExplosion() {
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        showOverlay();
        try { if (window.confetti) window.confetti({ particleCount: 180, spread: 70 }); } catch (_) {}
...
const onTap = (ev) => {
  ...
  if (count >= target) {
    if (state === States.FOUR) {
      ...
      animatePopStrong();
      setTimeout(() => { animateShake(); setTimeout(() => transitionTo(States.THIRTY_EIGHT), 220); }, 140);
    } else {
      finalExplosion();
    }
  } else {
    setTimeout(() => { blockingTap = false; }, 120);
  }
};
...
function resetAll() {
  ...
  heartBtn.classList.remove("pop", "pop-strong", "shake", "explode");
  clearParticles();
  state = States.FOUR; count = 0; updateInstruction(); updateCounter(); blockingTap = false;
  heartBtn.style.visibility = ""; instructionEl.style.visibility = ""; counterEl.style.visibility = "";
}
